...
class ExternalizingPredictorsBlend(nn.Module):
    ...
    def __init__(
        ...
        history_init = [init_bias_value] * self.history_size
        # self.history = deque(history_init, maxlen=history_size if history_size > 0 else None)
        self.history = history_init.copy()
    ...
    def forward(self, x_raw: torch.Tensor) -> torch.Tensor:
        ...
        if self.history_size > 0 and self.history_weight > 0:
            ...
            for pred_item in preds_list:
                ...
                self.history.pop(0)
                sliding_median_20_last = float(np.median(self.history[-20:]))
                sliding_median_10_last = float(np.median(self.history[-10:]))
                smoothed_pred_item = (sliding_median_10_last + sliding_median_20_last) / 2
